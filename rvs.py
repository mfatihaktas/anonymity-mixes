...
#     RV.__init__(self, m=U*loc, M=float("Inf") )
...
class DUniform(): # Discrete
	def __init__(self, lb, ub):
		RV.__init__(self, m=lb, M=ub)

	def __str__(self):
		return "DUniform[{}, {}]".format(self.m, self.M)

	def mean(self):
		return (self.M + self.m)/2

	def pdf(self, x):
		return float(1/(self.M - self.m + 1) )

	def sample(self):
		return random.randint(self.m, self.M)
...
class BoundedZipf():
	def __init__(self, lb, ub, a=1):
		...
		self.v = numpy.arange(self.m, self.M+1) # values
	...
	def __str__(self):
		return "BoundedZipf([{}, {}], a= {})".format(self.m, self.M, self.a)

	def pdf(self, x):
		return self.dist.pmf(x)

	def cdf(self, x):
		# if x < self.m: return 0
		# elif x >= self.M: return 1
		# else:
		#   return sum(self.p[:(x-self.m+1) ] )
...
class NegBinomial():
	def __init__(self, num_succ, p):
		RV.__init__(self, m=num_succ, M=float("Inf") )
		self.p = p

		self.dist = scipy.stats.nbinom(num_succ, p)

	def __str__(self):
		return "NegBinom[num_succ= {}, p= {}]".format(self.m, self.p)

	def cdf(self, x):
		return self.dist.cdf(x - self.m)
